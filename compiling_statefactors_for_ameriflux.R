...
library(readxl)
sites <- read_excel("AmerifluxDataAvailability.xlsx")
...
library(raster)
sf <- raster("Beck_KG_V1_present_0p0083.tif")
extent(sf)
crs(sf)

png("statefactors_climate.png",13,8,
...
# soil organic carbon:
sf <- raster("NACP_MSTMIP_UNIFIED_NA_SOIL_MA_1242/data/Unified_NA_Soil_Map_Topsoil_Organic_Carbon.tif")
...
png("statefactors_soil_topsoil_organic_carbon).png",13,8,
    units = "in",res = 600, pointsize=20, family= "helvetica")
plot(sf,legend = F)
points(sites_name_lat_lon$lon,sites_name_lat_lon$lat, pch = 19, cex = 0.5) #pch = 19: solid circle
...
sf <- raster("NACP_MSTMIP_UNIFIED_NA_SOIL_MA_1242/data/Unified_NA_Soil_Map_Subsoil_Organic_Carbon.tif")

xy <- cbind(sites_name_lat_lon$lon,sites_name_lat_lon$lat)
sf_xy <- extract(sf,SpatialPoints(xy))
sites_name_lat_lon_sf[,6] <- sf_xy

png("statefactors_soil_subsoil_organic_carbon).png",13,8,
    units = "in",res = 600, pointsize=20, family= "helvetica")
plot(sf,legend = F)
points(sites_name_lat_lon$lon,sites_name_lat_lon$lat, pch = 19, cex = 0.5) #pch = 19: solid circle
...
library(rgeos)
library(raster)
sf <- readOGR("GMNA_SHAPES/", layer = 'Geologic_units') #Large SpatialPolygonsDataFrame
summary(sf)
class(sf)
head(sf@data)
crs(sf)
...
st_layers("GMNA_SHAPES/")

sf <- st_read("GMNA_SHAPES/",layer = 'Geologic_units')
